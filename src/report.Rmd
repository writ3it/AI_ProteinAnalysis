...
fill_gaps<- function(id, colName, colVal){

  bad <- is.na(colVal)
  ids <- id[bad]
  #vals <- replacements %>% filter( any( res_name %in% ids ) ) # %>% select(!!treat_string_as_col(colName))
  print(ids)
  colVal[bad]<- unlist(vals, use.names = FALSE)
