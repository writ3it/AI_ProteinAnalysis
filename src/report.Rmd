...
head -n 100K data/all_summary.csv > data/big_summary.csv
```
...
Description of ligand contains few diffrent datatypes. These are mainly numbers like descriptive statistics of geometry. Because each ligand has diffrend shape some statistics don't appear. Some attributes contains data packed at JSON format. res_name attribute contains target class which is ligand name. Description of geometry need some knowladge about crystallography and PDB data formats. My knowladge is little so analysis isn't big succes, but example of work with R machine learning and analysis tools. 
...
stats <- data %>% select(attrib.legal.numeric) %>% summarise_all(funs(min,max))
attrib.empty <- lapply(attrib.legal,function(col){
  .max <- stats[1,paste(col,"_max",sep="")]
  .min <- stats[1,paste(col,"_min",sep="")]
  if (is.na(.max) || is.na(.min) || .max == .min){
   
    return (col)
  }
  return ()
})
attrib.empty[sapply(attrib.empty,is.null)]<-NULL
```
...
print(paste("Number of legal attributes: ",length(attrib.legal)))
...
data.train <- data %>% select_(.dots=c(attrib.exp_02,setdiff(colnames(data),c(attrib.exp_01,attrib.empty,attrib.target_class,attrib.illegal,attrib.local, attrib.dict))))
...
```{r correlation}
correlation <- cor(data.train,method="spearman")
...
for (target in attrib.reg.targets){
  ...
  fit <- train( dict_atom_non_h_count  ~ . ,
    ...
    print(ggplot(fit) + ggtitle(target)+ theme_bw())
    ...
    print(paste(target," RSquared= ",r2))
      ...
      fit <- train( dict_atom_non_h_electron_sum  ~ . ,
                 data=training,
                 method="lm",
                 metric="RMSE",
                 trControl = ctrl,
...
```{r remove_strongs}
strongCorrelated <- findCorrelation(correlation,cutoff=0.79)
cn <- colnames(correlation)
featuresNames <- cn[strongCorrelated]
data.train <- data.train %>% select_(.dots=setdiff(colnames(data.train),featuresNames))
```
...
data.train <- data.train %>% select_(.dots=setdiff(colnames(data.train),zeroVarNames))
```
### Training
```{r classification}
  partition<-0.8
  ...
  grid <- expand.grid(
    mtry = seq(1,floor(sqrt(folds)),by=1)
  )
...
### Accuracy
```{r acc}
  print(ggplot(fit) + ggtitle("Accurency ligands prediction")+ theme_bw())
```
